...
<body>
    ...
    <script>
        ...
        // Script para hacer rotar las categorías en el Logo 1 por 1 de forma automática
        document.addEventListener("DOMContentLoaded", function() {
            const texts = document.querySelectorAll('.rotating-text');
            if (texts.length <= 1) return; // Si hay 0 o 1 categoría, no hace falta rotar
            ...
            setInterval(() => {
                ...
                // 1. Enviamos el texto actual hacia arriba (salida)
                currentText.classList.remove('active');
                currentText.classList.add('exit');

                // Limpiamos su estado después de que termine su animación de salida
                setTimeout(() => {
                    currentText.classList.remove('exit');
                }, 600);

                // 2. Calculamos el índice del siguiente texto
                currentIndex = (currentIndex + 1) % texts.length;

                // 3. Traemos el nuevo texto desde abajo hacia el centro
                const nextText = texts[currentIndex];
                nextText.classList.add('active');

            }, 2500); // Cambia cada 2.5 segundos (2500ms)
        });
    </script>
</body>
