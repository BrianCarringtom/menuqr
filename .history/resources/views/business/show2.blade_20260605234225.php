...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Fredoka:wght@500;700&family=Poppins:wght@400;600;700&display=swap" rel="stylesheet">
    ...
    <style>
        ...
        *::after {
        ...
        .brand-header {
            ...
            background: linear-gradient(135deg, #ff007f 0%, #ff758c 100%);
        ...
        .category-giant-card:nth-child(4n+1)::before { background: var(--pinata-orange); }
        .category-giant-card:nth-child(4n+2)::before { background: var(--pinata-pink); }
        .category-giant-card:nth-child(4n+3)::before { background: var(--pinata-cyan); }
        .category-giant-card:nth-child(4n+4)::before { background: var(--pinata-yellow); }

        .category-giant-card:nth-child(4n+1) .img-blob-frame { background: var(--pinata-orange); }
        .category-giant-card:nth-child(4n+2) .img-blob-frame { background: var(--pinata-pink); }
        .category-giant-card:nth-child(4n+3) .img-blob-frame { background: var(--pinata-cyan); }
        .category-giant-card:nth-child(4n+4) .img-blob-frame { background: var(--pinata-yellow); }
        ...
        /* EFECTOS DINÁMICOS DE SELECCIÓN */
        ...
        .category-giant-card.active:nth-child(4n+1) { border-color: var(--pinata-orange); box-shadow: 0 0 15px rgba(255, 111, 0, 0.2); }
        .category-giant-card.active:nth-child(4n+2) { border-color: var(--pinata-pink); box-shadow: 0 0 15px rgba(255, 0, 127, 0.2); }
        .category-giant-card.active:nth-child(4n+3) { border-color: var(--pinata-cyan); box-shadow: 0 0 15px rgba(0, 180, 216, 0.2); }
        .category-giant-card.active:nth-child(4n+4) { border-color: var(--pinata-yellow); box-shadow: 0 0 15px rgba(255, 214, 0, 0.2); }
        ...
        /* Icono de cierre corregido especificando exclusión del buscador */
    ...
    <div class="products-flux-wrapper">
        @forelse ($user->categories as $index => $category)
            <div class="products-panel" id="panel-{{ $index }}">
                ...
                @empty
                    <p style="color: var(--text-muted); font-size: 13.5px; text-align: center; padding: 30px 10px; width: 100%;">
    ...
    <script>
        function selectCategory(index) {
            ...
            if (isAlreadyActive) {
                ...
                if(selectedPanel) selectedPanel.classList.remove('show-active');
                ...
                if(selectedPanel) selectedPanel.classList.add('show-active');
        ...
        function restablecerTarjetasOcultas() {
            ...
            document.querySelectorAll('.category-giant-card').forEach(card => {
                card.style.display = 'flex';
                card.classList.remove('hidden-by-active', 'active');
            });
        }
        ...
        function buscarEnTiempoReal() {
            ...
            if (query.length > 0) {
                ...
                panels.forEach((panel) => {
                    // Extraer índice directamente del ID del panel
                    const index = panel.id.replace('panel-', '');
                    const card = document.getElementById(`giant-card-${index}`);
