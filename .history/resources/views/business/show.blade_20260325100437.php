...
<body class="bg-black">
            ...
            @forelse ($user->categories as $index => $category)
                ...
                <div class="mb-10 border-b border-gray-700 pb-6">
                    ...
                    <button onclick="toggleMenu({{ $index }})"
                        ...
                        <!-- ICONO FLECHA -->
                        <span id="icon-{{ $index }}" class="accordion-icon text-yellow-400 text-2xl">
                            ➤
                        </span>
                    </button>

                    <!-- CONTENIDO -->
                    <div id="content-{{ $index }}" class="accordion-content mt-4">
    ...
    <!-- 🔥 SCRIPT ACORDEÓN -->
    <script>
        // Abrir última categoría guardada al cargar
        document.addEventListener('DOMContentLoaded', () => {
            const openIndex = localStorage.getItem('openCategory');
            if (openIndex !== null) {
                toggleMenu(Number(openIndex), true);
            }
        });

        function toggleMenu(index, skipSave = false) {
            ...
            allContents.forEach((content, i) => {
                ...
                if (i === index) {
                    if (content.style.maxHeight && content.style.maxHeight !== "0px") {
                        ...
                        if (!skipSave) localStorage.removeItem('openCategory');
                    } else {
                        // Abrir la categoría
                        content.style.maxHeight = content.scrollHeight + "px";
                        icon.style.transform = "rotate(90deg)";
                        if (!skipSave) localStorage.setItem('openCategory', index);
                    }
                } else {
