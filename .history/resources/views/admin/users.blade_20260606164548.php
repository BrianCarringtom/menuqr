...
@section('content')
    <style>
        ...
        .custom-table {
            width: 100%;
            min-width: 950px; /* Se incrementó un poco por la nueva columna */
        ...
        .row-expired {
            background-color: #ef4444 !important; /* Rojo intenso/alerta */
            color: #ffffff !important;
        }
        .row-expired td, .row-expired a {
            color: #ffffff !important;
        }
            ...
            <table class="custom-table">
                ...
                <tbody>
                    @php
                        ...
                        // Separamos los usuarios en dos colecciones en caliente basados en los 29/30 días
                        foreach ($users as $user) {
                            $daysPassed = \Carbon\Carbon::parse($user->created_at)->diffInDays(\Carbon\Carbon::now());
                            if ($daysPassed >= 29) {
                                $expiredUsers[] = ['data' => $user, 'days' => 30 - $daysPassed];
                            } else {
                                $normalUsers[] = ['data' => $user, 'days' => 30 - $daysPassed];
                            }
                        }

                        // Juntamos poniendo primero los vencidos / por vencer
                    ...
                    @foreach ($sortedUsers as $item)
                        @php 
                            ...
                            $daysLeft = $item['days'];
                        @endphp
                        <tr class="{{ $daysLeft <= 1 ? 'row-expired' : '' }}">
                            <td>
                                <form action="/admin/users/{{ $user->id }}/renew" method="POST" style="margin:0; padding:0;">
                                    @csrf
                                    @method('PUT')
                                    <button type="submit" class="btn-renew">Renovar</button>
                                </form>
                            </td>
                            <td>{{ $user->id }}</td>
                            <td>{{ $user->name }}</td>
